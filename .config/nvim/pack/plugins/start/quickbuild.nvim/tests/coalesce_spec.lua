...
describe("coalesce", function()
  ...
  describe("coalesce", function()
    it("merges diagnostics at same location", function()
      ...
      assert.equals("error: binary '+' not defined | note: with template arguments", result[1].message)
    end)
    ...
    it("keeps highest severity", function()
      ...
      assert.equals(1, #result)
      assert.equals(vim.diagnostic.severity.ERROR, result[1].severity)
      assert.equals("error: unused variable | syntax error", result[1].message)
    end)
    ...
    it("preserves message order", function()
      ...
      local result = coalesce.coalesce(input)
      assert.equals(1, #result)
      assert.equals("error: first | second | third", result[1].message)
    end)
    ...
    it("handles multiline MSVC template error", function()
      ...
      local result = coalesce.coalesce(input)
      assert.equals(1, #result)
      assert.equals("error: binary '+': type does not define operator | note: with template arguments | note: [A=int, B=char]", result[1].message)
    end)
